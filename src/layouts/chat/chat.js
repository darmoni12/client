...
function Chat({ userId, isAdmin, userImage }) {
    ...
    socket.on('message', (message) => {
      if (forMe(message.dst)) {
        setMessages(
          messages.concat([{
            text: message.text,
            type: message.dst == "admin" ? 0 : 1,
            image: message.dst == "admin" ? userImage : adminImage,
          }])
        );
      }
    })
  ...
  useEffect(() => {
    axios.post(`http://localhost:2400/message/getMessages`, { userId }, { withCredentials: true })
      .then(res => res.data.msg)
      .then((res) => {
        console.log(res)
        setMessages(res.map(x => ({
          type: x.idTypeIsSender ? 0 : 1,
          image: x.idTypeIsSender ? userImage : adminImage,
          text: x.text
        })
        ))
      });
